...
const FormModal = ({ isOpen, onClose, onSave }) => {
  ...
  const validateForm = () => {
    ...
    if (!form.context) newErrors.context = "Context is required"
    setErrors(newErrors);
        ...
        <ModalBody>
          <VStack spacing={4}>
            ...
            <FormControl isInvalid={!!errors.context}>
              ...
              <ChakraSelect
                name="context"
                value={contextOptions.find((opt) => opt.value === form.context)}
                onChange={(opt) => handleChange({ target: { name: 'context', value: opt.value } })}
                options={contextOptions}
                styles={selectStyles}
              />
              <FormErrorMessage>{errors.context}</FormErrorMessage>
            </FormControl>
          </VStack>
        </ModalBody>
        <ModalFooter>
