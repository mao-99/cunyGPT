...
collection = client["classRecommender"]["simplePantryCollection"]

# Load CSV file
data = pd.read_csv('nyFoodPantries.csv')
...
for index, row in data.iterrows():
    ...
    #Borough,Name,Address,ZipCode,Phone,Fax,Hours,Type
    # Combine relevant fields to create a document for embedding
    document_text = f"{row['Borough']} {row['Name']} {row['Address']} {row['Phone']} {row['Fax']} {row['Hours']} {row['Type']} {row['ZipCode']}"
